...
namespace controllers{
    ...
    void Wbc::kin_wbc(Robot_info &robot,Gait_info &gait,Swing_info &swing,double dt)
    {
        ...
        {
            int stand_num = 0;
            J[0].resize(3*contact_num,18);
            Jdtqdt[0].resize(3*contact_num);
            //传入量
            for(int i=0;i<4;i++)
            {
            ...
            if( stand_num != contact_num ) { ROS_ERROR("mode is stand: num %d != contact_num %d",stand_num,contact_num);}
            //中间变量
            Eigen::MatrixXd J1_pre_inv_(18, 3 * contact_num);
            utils::WeightedInverse(J[0],robot.M_q_inv,J1_pre_inv_);
            N_pre = Eigen::MatrixXd::Identity(18, 18) - J1_pre_inv_ * J[0];
            //目标量
            q_ddt_cmd[0] = J1_pre_inv_ * ( -Jdtqdt[0] );   
        }else{ //没有支撑腿
        ...
        // 1. 提取当前的俯仰角 (theta) 和 偏航角 (psi)
        double theta = robot.euler(1);
        double psi   = robot.euler(2);
        ...
        // 3. 构建公式 (4.38) 中的线性化转换矩阵
        Eigen::Matrix3d transform_mat;
        transform_mat << c_theta * c_psi, -s_psi, 0,
                        c_theta * s_psi,  c_psi, 0,
                        -s_theta,          0,     1;

        x_delta_cmd[1] = transform_mat * ( -robot.euler );
        ...
        if( contact_num != 4 )//有摆动腿
        {
            ...
            J[3].resize(3*(4-contact_num),18);
            Jdtqdt[3].resize(3*(4-contact_num));
            x_delta_cmd[3].resize(3*(4-contact_num));
            x_dt_cmd[3].resize(3*(4-contact_num));
            x_ddt_cmd[3].resize(3*(4-contact_num));
            J_pre[3].resize(3*(4-contact_num),18);
            for(int i=0;i<4;i++)
            {
            ...
            if( swing_num != (4-contact_num) ) { ROS_ERROR("swing: num %d != nocontact_num %d",swing_num, (4-contact_num));}
            x_ddt_cmd[3] = swing_kp * ( x_delta_cmd[3] ) 
                                + swing_kd * ( x_dt_cmd[3] - x_dt_now );
            //中间变量
            J_pre[3] = J[3] * N_pre;
            Eigen::MatrixXd J4_pre_inv_(18, 3 * swing_num);
            utils::WeightedInverse(J_pre[3],robot.M_q_inv,J4_pre_inv_);
            //目标量 
            q_delta_cmd[3] = q_delta_cmd[2] + J4_pre_inv_ * ( x_delta_cmd[3] - J[3]*q_delta_cmd[2] );
            q_dt_cmd[3] = q_dt_cmd[2] + J4_pre_inv_ * ( x_dt_cmd[3] - J[3]*q_dt_cmd[2] );
            q_ddt_cmd[3] = q_ddt_cmd[2] + J4_pre_inv_ * ( x_ddt_cmd[3] - J[3]*q_ddt_cmd[2] - Jdtqdt[3] );
    ...
    void Wbc::wbic(Robot_info &robot,Gait_info &gait,Wbc_info &wbc)
    {
        ...
        //CE
        dyn_CE.resize(6,6+3*contact_num); dyn_CE.setZero();
        qp_CE.resize(6+3*contact_num,6); qp_CE.setZero();

        dyn_CE.block<6,6>(0,0) = robot.M_q.block<6,6>(0,0); //q
        Eigen::MatrixXd J_c_T;
        J_c_T.resize(6,3*contact_num); J_c_T.setZero();
        ...
        f_mpc.resize(3*contact_num); f_mpc.setZero();
        int stand_num = 0;
        for(int i=0;i<4;i++)
        {
        ...
        //CI
        CA.resize(5*contact_num,3*contact_num); CA.setZero();
        UB.resize(5*contact_num); UB.setZero();
        LB.resize(5*contact_num); LB.setZero();
        ...
        dyn_CI.resize(2*5*contact_num,6+3*contact_num); dyn_CI.setZero();
        qp_CI.resize(6+3*contact_num,2*5*contact_num); qp_CI.setZero();

        dyn_CI.block(0,6,5*contact_num,3*contact_num) = -CA;
        dyn_CI.block(5*contact_num,6,5*contact_num,3*contact_num) = CA;
        qp_CI.noalias() = dyn_CI.transpose();
        //ci
        qp_ci.resize(2*5*contact_num); qp_ci.setZero();

        qp_ci.segment(0,5*contact_num) = UB - CA * f_mpc;
        qp_ci.segment(5*contact_num,5*contact_num) = CA * f_mpc - LB;
        ...
        eigenToQuadProgMat(qp_CE,quadprog_CE);   eigenToQuadProgVec(qp_ce,quadprog_ce0);
        eigenToQuadProgMat(qp_CI,quadprog_CI);   eigenToQuadProgVec(qp_ci,quadprog_ci0);
        ...
        end_f.resize(3*contact_num);
        end_f = f_mpc + qp_result.segment(6,3*contact_num);
        end_q_ddt = q_ddt_cmd[3];
        end_q_ddt.head(6) += qp_result.head(6);

        Eigen::MatrixXd end_J_T;
        end_J_T.resize(18,3*contact_num);
        end_J_T = J[0].transpose();
    ...
    void Wbc::debug(Wbc_info &wbc)
    {
        // ---------------------------------------------
        // task1 ：kin_wbc
        // ---------------------------------------------
        
        // ROS_INFO_STREAM("q_ddt_cmd[3]: \n" << q_ddt_cmd[3]);
